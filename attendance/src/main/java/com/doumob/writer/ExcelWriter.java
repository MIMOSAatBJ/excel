...
public class ExcelWriter {
	private String[] header = { "序号", "姓名", "日期", "星期", "首次打卡", "最后打卡", "时长",
			"是否加班" };
	...
	public void writeData(Sheet sheet, List<Pair> data) {
		...
		for (int i = 0; i < data.size(); i++) {
			Pair p = (Pair) data.get(i);
			if(DateUtil.isSameDay(p.getRecordDay(), DateUtil.getStandarDate())){
			//创建行
